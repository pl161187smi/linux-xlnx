...
#define FMRFB_PIXCLK_GEN_DEVS 8

static int (*fmrfb_hw_pixclk_set_fn[FMRFB_PIXCLK_GEN_DEVS])(unsigned long);
static bool fmrfb_hw_pixclk_init;

#if defined(CONFIG_FB_FMR_PIXCLK_ZYNQ_PS)

#define FMRFB_PIXCLK_ZYNQ_PS 1

#include <linux/io.h>
#include <linux/errno.h>

int fmrfb_hw_pixclk_set_zynq_ps(unsigned long pixclk_khz)
{
...
#endif /* #if defined(CONFIG_FB_FMR_PIXCLK_ZYNQ_PS) */

#if defined(CONFIG_FB_FMR_PIXCLK_LOGICLK)

#define FMRFB_PIXCLK_LOGICLK 2
...
int fmrfb_hw_pixclk_set_logiclk(unsigned long pixclk_khz)
{
#ifdef CONFIG_OF
...
#endif /* #if defined(CONFIG_FB_FMR_PIXCLK_LOGICLK) */

#if defined(CONFIG_FB_FMR_PIXCLK_SI570)

#define FMRFB_PIXCLK_SI570 3

#include <linux/i2c/si570.h>

int fmrfb_hw_pixclk_set_si570(unsigned long pixclk_khz)
{
...
#endif /* #if defined(CONFIG_FB_FMR_PIXCLK_SI570) */


bool fmrfb_hw_pixclk_supported(int id)
{
	if (!fmrfb_hw_pixclk_init) {
#if defined(FMRFB_PIXCLK_ZYNQ_PS)
		fmrfb_hw_pixclk_set_fn[FMRFB_PIXCLK_ZYNQ_PS] =
			fmrfb_hw_pixclk_set_zynq_ps;
#endif
#if defined(FMRFB_PIXCLK_LOGICLK)
		fmrfb_hw_pixclk_set_fn[FMRFB_PIXCLK_LOGICLK] =
			fmrfb_hw_pixclk_set_logiclk;
#endif
#if defined(FMRFB_PIXCLK_SI570)
		fmrfb_hw_pixclk_set_fn[FMRFB_PIXCLK_SI570] =
			fmrfb_hw_pixclk_set_si570;
#endif
		fmrfb_hw_pixclk_init = true;
	}

	return fmrfb_hw_pixclk_set_fn[id] ? true : false;
}

#if !defined(CONFIG_FB_FMR_PIXCLK)

int fmrfb_hw_pixclk_set(int id, unsigned long pixclk_khz)
{
...
int fmrfb_hw_pixclk_set(int id, unsigned long pixclk_khz)
{
	return fmrfb_hw_pixclk_set_fn[id](pixclk_khz);
}

#endif /* #if defined(CONFIG_FB_FMR_PIXCLK) */
