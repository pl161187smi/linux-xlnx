...
static void sii902x_setup(struct fb_info *fbi)
{
	...
	data[1] = 0x1770;//refresh * 100;
	tmp = (u8 *)data;
	for (i = 0; i < 8; i++)
	{
		dev_dbg(&sii902x.client->dev, "0x%x=0x%x\n", i, tmp[i]);
...
static int myir_get_of_property(void)
{
	...
	printk(KERN_DEBUG "AMX [%s] mode_str %s", __func__, mode_str);
	printk(KERN_DEBUG "AMX [%s] bits_per_pixel %d", __func__, bits_per_pixel);
		...
		const struct i2c_device_id *id)
{
	...
	if (sii902x.client->irq) {
		...
		if (ret < 0){
			printk(KERN_DEBUG "Sii902x: cound not create sys node for fb name\n");
						dev_warn(&sii902x.client->dev,
		...
		if (ret < 0){
			printk(KERN_DEBUG "Sii902x: cound not create sys node for cable state\n");
						dev_warn(&sii902x.client->dev,
	...
	printk(KERN_DEBUG "AMX probe 1");
	myir_get_of_property();
	printk(KERN_DEBUG "AMX probe 2");
	sii902x.waiting_for_fb = true;
	printk(KERN_DEBUG "AMX probe 3");
	fb_register_client(&nb);
	printk(KERN_DEBUG "AMX probe 4");
